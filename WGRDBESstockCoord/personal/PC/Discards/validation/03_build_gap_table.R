build_gap_table <- function(df, thresh_val = 50) {
  ...
  gap_table <- df %>%
    filter(Status != paste("✓ SAFE : >", thresh_val, " Coverage"))%>%
    ...
    mutate(
      Action_Required = case_when(
        ...
        Status == paste("WARNING: <", thresh_val, " Coverage") ~ "⚠️ POOL: Group with adjacent quarters",
        TRUE ~ "❌ BORROW: Search similar metier/quarter"
      )
    ) %>%
    as.data.frame()

 fwrite(gap_table, "outputs/gap_table.csv")
