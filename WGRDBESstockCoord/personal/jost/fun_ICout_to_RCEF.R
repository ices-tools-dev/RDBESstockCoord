...
ICout_RCEF <- function(dat_path, 
  ...
  write.csv(sd, paste0(dat_path, "/tmp/sd.csv"), row.names = F, quote = F)
  write.csv(si, paste0(dat_path, "/tmp/si.csv"), row.names = F, quote = F)
  write.csv(hi, paste0(dat_path, "/tmp/hi.csv"), row.names = F, quote = F)
