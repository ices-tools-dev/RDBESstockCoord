...
dat_path <- ".../intercatch"
out_path <- ".../rcef_intercatch"

stock_relation <- data.frame(Species = c(rep("BLL", 4),
                                         "COD"),
                             FishingArea = c("27.3.c.22", "27.3.b.23", "27.3.d.24", "27.3.d.25",
                                             "27.3.a.21"),
                             Stock = c(rep("bll.27.22-32", 4),"cod.27.3.a.21"), 
                             Year = 2023,
                             workingGroup  = "WGBFAS"
                             )
...
convExcahcnge <- function(dat_path = getwd(), #folder with intercatch echange format files
  ...
  codes_aph <- merge(codes_FAO, codes_aph, by = "Description")
  ...
  si_names <- c("RecordType","Country","Year", "SeasonType", "Season", "Fleet",
                ...
                "SamplesOri00gin", "QualityFlag", "UnitCaton", "Caton", "OffLandings", 
  ...
  #attach aphiaId
  si <- merge(si, codes_aph[, c("Species", "speciesCode")], by = c("Species"))  
  
  #merge with code list for the stock area and wg
  si <- merge(si, stock_relation, by = c("Species", "FishingArea", "Year"))  
  ...
  sd <- merge(sd, codes_aph[, c("Species", "speciesCode")], by = c("Species"))  
  sd <- merge(sd, stock_relation, by = c("Species", "FishingArea", "Year"))  
  ...
  hi <- merge(hi, unique(stock_relation[, c("Year", "FishingArea", "workingGroup")]), 
                         by = c("Year", "FishingArea"))  
  
  #order and output format
  hi <- hi[order(hi$key), ]
  effort <- data.frame(vesselFlagCountry = hi$Country,
                       year = hi$Year,
                       workingGroup = hi$workingGroup,
                       quarter = hi$Season,
                       area = hi$FishingArea,
...
convExcahcnge(dat_path = dat_path, output_format = "to_environment")
