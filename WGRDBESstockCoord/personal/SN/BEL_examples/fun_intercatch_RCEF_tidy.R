...
#'
#' @examples
convExchange_tidy2 <- function(dat_path = getwd(),
                               ...
                               file_suffix = paste0("_RCEF_v", getOption("RCEF_version"))) # Delayed evaluation makes it okay!
{
  ...
  ## fixed relations
  effort_relation <- data.frame(UnitEffort = c("dop", "kWd", "kwd", "fd", "hf", "kh", "NoV", "tr"),
  ...
  catches <- data.frame(vesselFlagCountry = si$Country,
    ...
    tidyr::pivot_longer(
      cols = dplyr::any_of(c("WGWeight", "OfficialWeight")),  # <- robust
      names_to = "variableType",
      values_to = "total"
    ) %>%
    dplyr::filter(!is.na(total)) %>%
    dplyr::relocate(comment, .after = dplyr::last_col())

  catches <- catches %>%
    dplyr::mutate(
      metier6 = dplyr::case_when(
        is.na(metier6) ~ NA_character_,                    # als het ontbreekt → NA
        tolower(metier6) == "fleet" ~ fleetValue,          # als het 'fleet' is → vervangen
        TRUE ~ metier6                                    # anders → ongewijzigd laten
      )
    )
  ...
  hi <- hi %>%
    dplyr::group_by(Country, Year, quarter, Season, FishingArea,
  ...
  effort <- effort %>%
    dplyr::mutate(
      metier6 = ifelse(tolower(metier6) == "fleet", fleetValue, metier6)
    )
